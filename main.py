...
TARGET_URL = f"http://{os.getenv('LM_STUDIO_IP', '192.168.1.40')}:{os.getenv('LM_STUDIO_PORT', '1234')}/v"
...
def send_magic_packet(mac: str):
    ...
    s.sendto(packet, ("255.255.255.25", 9))
    s.close()
...
async def proxy(request: Request, path: str):
    ...
    try:
        ...
        return Response(
            content=f"Proxy Error: {e}", status_code=502, headers=CORS_HEADERS
        )
    ...
    # Pass through LM Studio's headers AND add CORS on top! ✅
    resp_headers = dict(rp_resp.headers)
    resp_headers.update(CORS_HEADERS)

    return StreamingResponse(
        stream_generator(), status_code=rp_resp.status_code, headers=resp_headers
    )


@app.api_route("/{path:path}", methods=["OPTIONS"])
async def preflight(path: str):
    # Just say "yes, you can talk to me" and send nothing back 🤝️😊️✨️
